...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast({
        title: 'Welcome back!',
        description: 'Successfully signed in to your MoodHub.',
      })
      navigate('/dashboard')
    }
    setLoading(false)
      ...
      >
        <CardHeader className="text-center pb-8">
          ...
          <CardDescription style={{ color: '#7c998a' }}>
            Sign in to your portal to continue your journey together
          </CardDescription>
        </CardHeader>
          ...
          <CardFooter className="flex flex-col space-y-6 px-6 pt-8">
            ...
            >
              <span className="relative z-10">
                {loading ? 'Entering the Garden...' : 'Enter the Garden'}
              </span>
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
            </Button>

            {/* Soft divider before signup link */}
            <div 
